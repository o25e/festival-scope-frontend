...
function vBars(labels, vals, hl, opt = {}) {
  ...
  if (opt.ref) {
    ...
    s += `<line x1="${padL}" y1="${yy}" x2="${W - padL}" y2="${yy}" stroke="${CHART.track}" stroke-dasharray="3 3"/>`
  }
...
export function getDetailHtml(item, A) {
  ...
  if (item.key === 'demand') {
    ...
    return (
      ...
      sec(
        2,
        '판단 근거 및 데이터',
        `<div class="vizbox">${vBars(MONTHS, R.monthly, eventMonthIndex, { ref: 100 })}<p class="vizcap">${R.name}의 최근 5년 월별 관광수요 지수(연평균 100 기준). 개최 월 ${displayDemandValue(eventMonth)}월은 ${displayDemandValue(v.mIdx)}로 연중 ${displayDemandValue(v.mRank)}위입니다.</p></div><div class="vizbox weekly-demand-card"><div class="weekly-demand-head"><h5>직전년도 개최월 주차별 방문객 수</h5><span class="weekly-demand-basis">${weeklyBasis}</span></div><div class="weekly-demand-layout"><div class="weekly-demand-chart"><span class="weekly-demand-unit">(${weeklyUnit})</span>${weeklyBars(weeklyData, weeklyUnit)}</div><div class="weekly-demand-insight"><span class="insight-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M6 18V11M12 18V6M18 18V9"/></svg></span><p>${weeklyPeak?.label || '-'}의 방문객이 가장 많았습니다. 축제 운영 시 해당 시기의 교통·주차 혼잡에 유의할 필요가 있습니다.</p></div></div></div><div class="access-summary-grid"><div class="access-summary-card transit-summary">${accessIcon('transit')}<div class="access-summary-copy"><div class="access-summary-label">대중교통 접근성</div><div class="access-summary-value">${transitSummary}</div><p>${stationSummary}</p></div></div><div class="access-summary-card parking-summary">${accessIcon('parking')}<div class="access-summary-copy"><div class="access-summary-label">확보 주차면수</div><div class="access-summary-value">${parkingDisplay}</div><p>${parkingSummary}</p></div></div></div><p class="parking-basis">${parkingBasis}</p>`,
      ) +
      sec(
        3,
        '결과 해석',
  ...
  if (item.key === 'link' && A.linkage) {
    ...
    return (
      ...
      sec(2, '판단 근거 및 데이터', `<div class="vizbox"><table class="dt"><tr><th>구분</th><th class="n">3km 이내</th><th class="n">3~5km</th><th class="n">5km 이내</th></tr>${categoryRows}</table></div><table class="dt" style="margin-top:12px"><tr><th>주요 연계 자원 (1km 이내)</th><th>구분</th><th class="n">거리</th></tr>${poiRows}</table>`) +
